    ...
    function openCheckoutModal() {
        ...
        if (cart.length === 0) {
            ...
            // Generate list items and join them into a string
            const listItems = cart.map(item => 
                `<li>${item.name} - £${item.price} x ${item.quantity} = £${(item.price * item.quantity).toFixed(2)}</li>`
            ).join(""); // Ensures no stray characters or spaces between <li> elements
        
            // Update modal content
            modalContent.innerHTML = `
                <h3>Checkout Summary</h3>
                <ul>
                    ${listItems}  <!-- This correctly nests the <li> elements within the <ul> -->
                </ul>
